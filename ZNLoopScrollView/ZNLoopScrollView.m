 ...
 */
@interface ZNCollectionCell : UICollectionViewCell

@property(nonatomic, strong) ZNLoadImageView *imageView;
@property(nonatomic, strong) UILabel         *titleLabel;
@property(nonatomic, assign) BOOL            isDragging;

@end

@implementation ZNCollectionCell

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.imageView = [[ZNLoadImageView alloc] init];
...
- (void)layoutSubviews {
    ...
    self.titleLabel.frame = CGRectMake(0, self.zn_height - 30, self.zn_width, 30);
    self.titleLabel.hidden = self.titleLabel.text.length > 0 ? NO : YES;
}
@end


@interface ZNLoopScrollView() <UICollectionViewDelegate, UICollectionViewDataSource> {
    ZNPageControl *_pageControl;
}
...
@implementation ZNLoopScrollView
...
+ (instancetype)loopScrollViewWithFrame:(CGRect)frame
    ...
    ZNLoopScrollView *loopView = [[ZNLoopScrollView alloc] initWithFrame:frame];
...
- (void)configCollectionView {
    ...
    [self.collectionView  registerClass:[ZNCollectionCell class]
...
- (void)configPageControl {
    if (self.pageControl == nil) {
        _pageControl = [[ZNPageControl alloc] init];
    ...
    self.pageControl.zn_size = size;
    
    if (self.alignment == kPageControlAlignCenter) {
        self.pageControl.zn_originX = (self.zn_width - self.pageControl.zn_width) / 2.0;
    } else if (self.alignment == kPageControlAlignRight) {
        self.pageControl.zn_rightX = self.zn_width - 10;
    }
    self.pageControl.zn_originY = self.zn_height - self.pageControl.zn_height + 5;
}
...
- (void)setAlignment:(ZNPageControlAlignment)alignment {
...
- (void)dealloc {
    NSLog(@"znloopscrollview dealloc");
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  ZNCollectionCell *cell =
      [collectionView dequeueReusableCellWithReuseIdentifier:kCellIdentifier
                                                forIndexPath:indexPath];

    //先取消之前的请求
    ZNLoadImageView *preImageView = cell.imageView;
    if ([preImageView isKindOfClass:[ZNLoadImageView class]]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.didSelectItemBlock) {
        ZNCollectionCell *cell = (ZNCollectionCell *)[self collectionView:collectionView cellForItemAtIndexPath:indexPath];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    int itemIndex = (scrollView.contentOffset.x +
                     self.collectionView.zn_width * 0.5) / self.collectionView.zn_width;
    ...
    CGFloat x = scrollView.contentOffset.x - self.collectionView.zn_width;
    NSUInteger index = fabs(x) / self.collectionView.zn_width;
    CGFloat fIndex = fabs(x) / self.collectionView.zn_width;
    
    if (self.didScrollBlock && fabs(fIndex - (CGFloat)index) <= 0.00001) {
        ZNCollectionCell *cell = (ZNCollectionCell *)[self collectionView:self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:itemIndex inSection:0]];
